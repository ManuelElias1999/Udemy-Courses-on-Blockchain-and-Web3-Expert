...
contract ArtToken is ERC721, Ownable{
    ...
    // smart Contrat Constructor
    constructor (string memory _name, string memory _symbol) ERC721(_name, _symbol) {}

    // NFT token counter
    uint256 COUNTER;

    // Princing ofNFT Token (price of the artwork)
    ...
    // Storage strcuture for keeping artiworks
    ...
    // Creation of a  random number (require for NFT toker properties)
    ...
    // Obtaining all created NTF tokens (artworks)
    function getArtWorks() public view returns (Art [] memory) {
        return art_works;
    }

    // Obtaining a users NFT tokens
